import { OmniSwitchClient } from '../src/client';
import { OmniSwitchApiError } from '../src/errors';
...
describe('OmniSwitchClient', () => {
  it('authenticates once, then reuses the cached token for a second call', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledTimes(3); // 1 auth + 2 resource calls, no re-auth
  ...
  it('retries exactly once with a fresh token on a 401, then succeeds', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledTimes(4); // auth, 401, re-auth, success
  ...
  it('throws a clear MFA_NOT_SUPPORTED error instead of silently returning a restricted token', async () => {
    ...
    const error: unknown = await client.getPayment('pay_1').catch((e) => e);
    expect(error).toBeInstanceOf(OmniSwitchApiError);
