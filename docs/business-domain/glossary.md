...
| **Circuit breaker** | Per-PSP failure tracking (`CLOSED`/`OPEN`/`HALF_OPEN`), shared across every replica via Redis, that temporarily stops routing traffic to a PSP that's failing repeatedly. See `ledger-and-settlement.md`. |
| **AMBIGUOUS** | A `Payment` status reached when a PSP call gets no response at all (not a decline — a timeout/network failure) and a same-provider retry also gets no response. Distinct from `FAILED`: this system genuinely doesn't know whether the PSP actually processed the charge, so it never falls back to a different PSP (that would risk a double charge). Has no automated resolution path today — see `payment-lifecycle.md`. |
| **PSP entitlement** | Which PSPs a merchant's charges are allowed to route through (`MerchantEntity.enabledPspProviders`, set via `PATCH /admin/merchants/:id/psp-entitlement`) — defaults to every PSP this system has an adapter for. A `preferredProvider` outside a merchant's entitlement is rejected (422), not silently routed to a different PSP. See `ledger-and-settlement.md#smart-psp-routing`. |
