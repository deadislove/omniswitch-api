 ...
 * Restored in afterAll so it doesn't leak into whichever e2e file the
 * same Jest worker runs next — `process.env` mutations are per-process,
 * so this can't leak across *different* concurrent workers either way,
 ...
 * shared Redis state across every e2e file the same Jest worker runs (each
 * worker gets its own Redis DB — test/setup-env.ts — but no flush between
 * files within one worker), so a leaked OPEN state here is the root cause of
...
describe('PSP bulkhead isolation (e2e)', () => {
  ...
  afterAll(async () => {
    ...
    // app.close() so that state doesn't leak into whichever e2e file the
  ...
  it('5 concurrent slow STRIPE calls against a bulkhead of 3 take meaningfully longer than 1 batch — the 4th and 5th queue for a permit', async () => {
