...
describe('Health-aware merchant throttling (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    // file the same Jest worker runs (each worker gets its own Redis DB
    // — test/setup-env.ts — but no flush between files within one
    // worker), so reset it here rather than assume a clean window.
  ...
  it('once this merchant is exposed to a degraded STRIPE, their subsequent charges hit a stricter limit than the base per-merchant ceiling', async () => {
