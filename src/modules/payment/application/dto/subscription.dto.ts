...
export class CreateSubscriptionDto {
  @ApiPropertyOptional({
    example: 'a1b2c3d4-...',
    description:
      'Subscribe using a pre-defined Plan (POST /plans) — amount/currency/interval/intervalCount are taken from the plan and any of those fields below are ignored. Either this or amount+currency+interval must be provided.',
  })
  @IsOptional()
  @IsUUID()
  planId?: string;
  ...
  @ApiPropertyOptional({
    example: 29.99,
    description: 'Amount charged per billing period, in major currency units. Required if planId is omitted.',
  })
  @IsOptional()
  ...
  @ApiPropertyOptional({
    example: 1,
    description:
      'Bill every N intervals — e.g. interval=month, intervalCount=3 bills quarterly. Defaults to 1. Ignored if planId is set.',
  })
  @IsOptional()
  @IsInt()
  @Min(1)
  ...
  @ApiProperty({
    ...
    description:
      'Stored payment method reference — every renewal charges this off-session, so (unlike a one-time charge) a card token alone is not accepted',
  })
  @IsString()
  @IsNotRawCardNumber()
  ...
  @ApiPropertyOptional({
    example: 14,
    description:
      'Trial length in days — no charge happens until the trial elapses. Omit/0 to charge the first period immediately.',
  })
  @IsOptional()
  @IsInt()
  @Min(0)
...
export class CancelSubscriptionDto {
  @ApiPropertyOptional({
    example: true,
    description:
      'true: keep billing through the current period, cancel when it ends. false/omitted: cancel immediately.',
  })
  @IsOptional()
  @IsBoolean()
  atPeriodEnd?: boolean;
...
export class ChangePlanDto {
  @ApiProperty({
    example: 'a1b2c3d4-...',
    description:
      "The Plan to switch to (POST /plans) — must belong to the same merchant, be active, and be in the same currency as the subscription's current amount.",
  })
  @IsUUID()
  planId: string;
}
...
export class SubscriptionResponseDto {
  ...
  @ApiProperty({
    example: false,
    description: 'If true, the subscription will stop (not renew) at the end of the current period',
  })
  cancelAtPeriodEnd: boolean;

  @ApiProperty({
    example: 0,
    description: 'Consecutive failed billing attempts for the current period — resets to 0 on a successful charge',
  })
  ...
  @ApiPropertyOptional({
    example: 5.5,
    description:
      "Unused credit from a prior downgrade, applied against a future period's charge — absent if there is none",
  })
...
export class ChangePlanResponseDto {
  ...
  @ApiPropertyOptional({
    example: 12.34,
    description:
      'The prorated amount actually charged for switching mid-period, if this was an upgrade. Mutually exclusive with creditIssued.',
  })
...
export class BillingSweepResultDto {
  ...
  @ApiProperty({
    example: 1,
    description: 'Subscriptions finalized as CANCELED (cancelAtPeriodEnd reaching its due date)',
  })
  canceled: number;

  @ApiProperty({
    example: 0,
    description: "Charge attempts that failed — see each subscription's failedAttempts/status for dunning progress",
  })
  failed: number;
}
