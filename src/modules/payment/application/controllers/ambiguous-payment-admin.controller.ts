...
import {
  ResolveAmbiguousPaymentDto,
  ListAmbiguousPaymentsQuery,
  AmbiguousPaymentSummaryDto,
  ResolvedAmbiguousPaymentResponseDto,
} from '../dto/ambiguous-payment.dto';
...
export class AmbiguousPaymentAdminController {
  ...
  @ApiOperation({
    summary:
      'List AMBIGUOUS payments — a PSP call that got no response at all, and a same-provider retry also got no response. Omit olderThanMinutes to list every currently AMBIGUOUS payment.',
  })
  ...
  async resolveAmbiguous(
    @Param('id') id: string,
    @Body() dto: ResolveAmbiguousPaymentDto,
    @Req() req: any,
  ): Promise<ResolvedAmbiguousPaymentResponseDto> {
